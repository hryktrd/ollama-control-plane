...
社内または個人管理下の複数PC上でOllamaを通じてローカルLLMを動かし、中央サーバがそれらをエージェントプールとして管理し、Claude CodeおよびOpenAI互換APIクライアントから一貫した方法で利用できるようにする。

対応モデルはQwen Coder、Gemma、Llama、Mistralなど、Ollama互換のモデルすべてを想定する。特定モデルへの依存は持たない。
